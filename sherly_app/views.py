...
def commande_list(request):
    ...
    if search_query:
        try:
            ...
            bl_data = bl_data.filter(
                Q(client__icontains=search_query) |
                Q(date_de_cmd__date=search_date) |
                Q(bon_livraison__date_de_bl__date=search_date)
            )
        except ValueError:
            #bl_data = bl_data.filter(client__icontains=search_query)
            bl_data = bl_data.filter(
                Q(client__icontains=search_query) |
                Q(bon_livraison__no_bl=search_query) |
                Q(date_de_cmd__icontains=search_query)|
                Q(bon_livraison__date_de_bl__icontains=search_query)  # Recherche exacte sur le champ no_bl
            )

    # Créer un QueryDict pour conserver les paramètres de requête
    ...
    return render(request, 'commande/liste.html', {'bl_data': bl_data, 'search_query': search_query, 'query_params': query_params.urlencode()})
...
def products_list(request):
    # Récupérer l'ID de la famille filtrée
    selected_category_id = request.GET.get('filterByFamille')
    products = Produit.objects.all().order_by('is_active', 'designation')

    # Filtrer les produits par famille si un ID de famille est fourni
    if selected_category_id:
        selected_category_id = int(selected_category_id)
        products = products.filter(famille_id=selected_category_id)
    ...
    # Créer un QueryDict pour conserver les paramètres de requête
    query_params = QueryDict(mutable=True)
    query_params['filterByFamille'] = selected_category_id

    # Set the number of items per page
    items_per_page = 8

    # Create a Paginator instance with filtered products
    ...
    return render(request, 'produits/liste.html', {'products': products,'famille_list':famille_list,'total_products':total_products,'selected_category_id': selected_category_id, 'query_params': query_params.urlencode()})
...
def generate_pdf_zip(request, bl_id, temp_dir):
    try:
        bon_livraison = Bon_Livraison.objects.get(pk=bl_id)
        societe = Societe.objects.first()
        table_bl = TABLE_BL.objects.first()
        ...
        logger.error(f"Bon Livraison {bl_id} not found")
    except Exception as e:
        logger.error(f"Error generating PDF for Bon Livraison {bl_id}: {e}")

def generate_zip(request):
    try:
        bon_ids_string = request.GET.get('bon_ids')
        bon_ids = json.loads(bon_ids_string)  # Convert string to list
        date_facture=request.GET.get('date_facture')

        temp_dir = tempfile.mkdtemp()
        zip_filename = f"NOTAS_DE_ENTREGA_DEL_{date_facture}.zip"
        zip_filepath = os.path.join(temp_dir, zip_filename)

        with zipfile.ZipFile(zip_filepath, 'w') as zipf:
            for bon_id in bon_ids:

                logger.info(f"Generating PDF for {bon_id}")
                pdf_file_path = generate_pdf_zip(request, bon_id, temp_dir)
                if pdf_file_path:
                    bon_livraison = Bon_Livraison.objects.get(pk=bon_id)
                    zipf.write(pdf_file_path, arcname=f"NOTA_ENTREGA_Nº_{bon_livraison.no_bl}.pdf")
                    #response['Content-Disposition'] = f'attachment; filename="NOTA_ENTREGA_Nº_{bon_livraison.no_bl}.pdf"'
